...
import { DEFAULT_NOTES_HTML, groupFields } from "./invoicePreviewConstants";

interface UseInvoicePreviewModalResult {
  items: BudgetItem[];
...
export function useInvoicePreviewModal({
  ...
  useEffect(() => {
    ...
    setGroupValues((prev) => {
      if (prev.length === 0) return vals;
      const filteredVals = prev.filter((v) => vals.includes(v));
  ...
  const closeDeleteConfirm = () => setIsConfirmingDelete(false);
