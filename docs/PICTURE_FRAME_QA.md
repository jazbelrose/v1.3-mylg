...
- Insert multiple frames and confirm they all exist (new inserts are offset so they don't overlap perfectly).
...
- With a frame that has an image, hold `Alt` and drag inside the frame to reposition the image crop.
- Save/reload and verify the crop position persists (object-position).
...
- Save slide, reload, and verify `imageSrc`, `radius`, `fit`, `border`, crop position (`positionX/positionY`), and rect (`x/y/width/height`) roundtrip.
- If S3 upload fails / no projectId, dropped images may use `blob:` URLs which won't survive reload (expected MVP fallback).

## Thumbnails
- Sidebar thumbnail and backend thumbnail render the picture frame (image clipped, radius/border/crop position respected).
